...
class TLSUtilsSpec extends AnyFunSpec {
  ...
  describe("To have Client authentication defined NEED"){
    ...
    val negotiableSession: NegotiateNewSession = NegotiateNewSession(None,None,Some(TLSClientAuth.Need),Some(sslParams))

    it("SSLParameters does not by default require client authentication when it is not passed to the constructor of HttpConnectionContext. In that case its default value is used by akka-http.") {
    ...
    it("SSLEngine must have ClientAuth defined Need after calling akka.stream.impl.io.TlsUtils.applySessionParameters with sslEngine and negotiableSession"){
